...
import { IStudyStatus, IStudyStatusService } from './interfaces/study-status.interface';
...
export class StudyStatusService implements IStudyStatusService {
  ...
  async findAll(userId: number, args?: any): Promise<StudyStatus[]> {
    const query = this.studyStatusRepository.createQueryBuilder('studyStatus')
      .where('studyStatus.userId = :userId', { userId });

    if (args?.subject) {
      query.andWhere('studyStatus.subject = :subject', { subject: args.subject });
    }

    if (args?.studyPlanId) {
      query.andWhere('studyStatus.studyPlanId = :studyPlanId', { studyPlanId: args.studyPlanId });
    }
  ...
  async findOne(id: number, userId: number): Promise<StudyStatus> {
    const studyStatus = await this.studyStatusRepository.findOne({ where: { id } });
