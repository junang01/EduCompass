...
import './bookSurvey2.css';
import { Link, useNavigate, useLocation } from "react-router-dom";
import { gql, useMutation } from "@apollo/client";

const CREATE_BOOK_RECOMMENDATION = gql`
  mutation CreateBookRecommendation($input: CreateBookRecommendationInput!) {
    createBookRecommendation(createBookRecommendationInput: $input) {
      id
      book {
        id
        title
...
const BookSurvey2Page: React.FC = () => {
  const [username, setUsername] = useState<string>("");
  const navigate = useNavigate();
  const location = useLocation();
  const { subject, detailSubject } = location.state || {};
  ...
  const [createBookRecommendation] = useMutation(CREATE_BOOK_RECOMMENDATION);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data } = await createBookRecommendation({
        variables: {
          input: {
            bookId: 1,
            subject,
      ...
      navigate("/bookSurveyResult", { state: { book: data.createBookRecommendation } });
      ...
      <main className="survey-page">
        <form className="survey-form">
          ...
          <div className="form-footer">
            ...
            <button type="button" onClick={() => navigate('/bookSurveyResult')}>
              {loading ? "추천 중..." : "제출"}
            </button>
          </div>
        </form>
...
export default BookSurvey2Page;
